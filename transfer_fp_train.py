...
def main(cfg: DictConfig):
    ...
    if cfg.interaction:
        ...
        with tqdm(range(episodes)) as pbar:
            for i in pbar:
                ...
                writer.log_metric("Total Discovered FI by train", len(total_fp), step=i)
                writer.log_metric("Discovered FI by train", len(env.sfp), step=i)
                writer.log_metric(
        ...
        if len(train_htwsp) != model.fc1.weight.size()[1]:
    ...
    test_htwsp = env.htwsp_1
    shared_weights_column = {
        train_i2b_dict[i]: test_i2b_dict[i] for i in (train_htwsp & test_htwsp)
    }
        ...
        for i in pbar:
            ...
            for t in count():
                ...
                if done:
                    break
            total_fp = total_fp | env.sfp
            writer.log_metric("Total Discovered FI by test", len(total_fp), step=i)
            writer.log_metric("Discovered FI by test", len(env.sfp), step=i)
            writer.log_metric(
    ...
    writer.log_artifact(os.path.join(os.getcwd(), "transfer_fp_train.log"))
